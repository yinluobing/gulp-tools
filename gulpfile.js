...
if (App_config.isDev) {
    var obj = {
        devPath: App_config.project_path + '/' + time.year + time.month + time.day + App_config.app_name + '/' + App_config.bulid_path,
        outPath: App_config.project_path + '/' + time.year + time.month + time.day + App_config.app_name + '/' + App_config.dist_path,
        desktopImg: App_config.ps_img_path,
    }

} else {
    var obj = {
        devPath: App_config.edit_path,
        outPath: App_config.edit_path + App_config.dist_path,
    }
}

/*********模板路径定义*********/
if (App_config.isPc) {
    var tplPath = App_config.pc_tpl_path;
} else {
    var tplPath = App_config.m_tpl_path;
}
...
var _dev = {
    ...
    version: '1.0',
    meta: { //                                              /
        devPath: obj.devPath, // 开发路径                                      build/
        outPath: obj.outPath, // 输出路径                                      dist/
        ...
        tplPath: tplPath, // 模板路径                                      build/tpl/ | build/tpl/
...
var _config = {
    copy: [ // 文件复制配置（不同的路径需要配置不同的对象）
        { // copy webapp/lib -> build/lib
            ...
            filePath: _dev.meta.devPath + _dev.meta.libPath,
            outPath: _dev.meta.outPath + _dev.meta.libPath,
            fileName: '**/*'
        },
         { // 字体文件
    ...
    html: {
        ...
        abandon: '!'+_dev.meta.devPath+_dev.meta.libPath+'**/*.html', // 不处理的文件
    },
    less: {
...
/*     console.log([_config.html.filePath + _config.html.fileName,_config.html.abandon]);
process.exit();   */  
/*********Html Inject contact模块合并**********/
gulp.task('html', function (cb) {
    pump([
        gulp.src([_config.html.filePath + _config.html.fileName,'!'+_dev.meta.devPath+'**/_*.html',_config.html.abandon]),
...
if (App_config.isDev) {
    ...
    _config.tpl = {
        filePath: _dev.meta.tplPath, // 待复制路径
        outPath: _dev.meta.devPath, // 目标路径
        fileName: '*' // 文件
    }
}
...
/*********gulp默认任务**********/
if (App_config.isDev) {
    if (!fs.existsSync(_dev.meta.devPath)) {
        utils.mkdirs(_dev.meta.devPath);
        utils.mkdirs(_dev.meta.outPath);
        /*********模板复制**********/
        utils.copy('./' + _config.tpl.filePath, _config.tpl.outPath);
    }
}
console.log(_config.copy);
